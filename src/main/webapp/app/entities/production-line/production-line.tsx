...
export const ProductionLine = () => {
  ...
  const productionLineList = useAppSelector(state => state.productionLine.entities);
  const loading = useAppSelector(state => state.productionLine.loading);
  const totalItems = useAppSelector(state => state.productionLine.totalItems);
  ...
  return (
    <div>
            ...
            <tbody>
              {productionLineList.map((productionLine, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/production-line/${productionLine.id}`} color="link" size="sm">
                      {productionLine.id}
                    </Button>
                  </td>
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/production-line/${productionLine.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/production-line/${productionLine.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
      ...
      {totalItems ? (
        <div className={productionLineList && productionLineList.length > 0 ? '' : 'd-none'}>
